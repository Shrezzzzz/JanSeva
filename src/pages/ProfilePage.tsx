    ...
    async function loadProfile() {
      ...
      try {
        ...
        const resolvedCount = issues.filter(
          (i: Issue) => i.status === 'Resolved' || i.status === 'Closed',
        ).length;
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div>
          <h2 className="font-display text-2xl text-[#0D0D0B] mb-4">Badges</h2>
          <BadgeGrid badges={profile.badges} />
        </div>
        <div>
          <h2 className="font-display text-2xl text-[#0D0D0B] mb-4">Recent Reports</h2>
          {myIssues.length > 0 ? (
            <div className="space-y-3">
              {myIssues.map((issue) => (
                <div key={issue.id} className="relative group">
                  <IssueCard
                    issue={issue}
                    onClick={(i) => navigate(`/track/${i.id}`)}
                  />
                  {/* Edit button — only show for the authenticated user's own issues */}
                  {authUser && (!issue.reporterId || issue.reporterId === authUser.id) && (
                    <button
                      onClick={(e) => { e.stopPropagation(); navigate(`/report/${issue.id}/edit`); }}
                      className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-white border border-[#E5E5E0] text-[#6F6F6F] hover:border-[#1A6B3C] hover:text-[#1A6B3C] transition-all shadow-sm"
